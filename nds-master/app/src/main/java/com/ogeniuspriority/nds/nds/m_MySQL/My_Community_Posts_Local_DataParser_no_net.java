 ...
 */
public class My_Community_Posts_Local_DataParser_no_net extends AsyncTask<Void, Void, Boolean> {
    Context c;
    ...
    public My_Community_Posts_Local_DataParser_no_net(Context c, ListView forum_whisper, SwipeRefreshLayout swipe_view) {
        this.c = c;
    ...
    protected void onPostExecute(Boolean isParsed) {
        ...
        if (isParsed) {
            ...
            if (My_Community_Posts_Array.size() == 0) {
                Toast.makeText(c, "No Data to display!", Toast.LENGTH_SHORT).show();
            }
    ...
    private Boolean parseData() {
        ...
        //-------------------
        Cursor myPosyts = db.GET_ALL_NEZA_COMMUNITY_POSTS_NO_DATA();
        ...
        for (int i = 0; i < myPosyts.getCount(); i++) {
            if (i == 0) {
                Urubuga_Services.lastId_Of_post_Retreived = Integer.parseInt(myPosyts.getString(1));
            }
            if (i == (myPosyts.getCount() - 1)) {
                Urubuga_Services.firstId_Of_post_Retreived = Integer.parseInt(myPosyts.getString(1));
            }
