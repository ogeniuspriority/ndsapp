 ...
 */
public class My_POSTS_Comments_data_parser_A extends AsyncTask<Void, Void, Boolean> {
    Context c;
    String jsonData;
    ...
    public My_POSTS_Comments_data_parser_A(Context c, String jsonData) {
        this.c = c;
                ...
                for (int i = 0; i < Notif_number_queries.length; i++) {
                    try {
                        ...
                        if (db.save_A_NOTIFICATION(uniqueID, Notif_title_queries[i],
                                ...
                                origin_remote_id_queries[i],"comments_on_posts",Orient_queries[i]
                               )) {
                            final int finalI = i;
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    Looper.prepare();
                                    try {
                                        final String theAvatarValue = new NDS_main().FindThisTrickyAvatar(origin_remote_id_queries[finalI], "notif_using_cmnts_on_posts");
                                        if (new NDS_main().renderThisAvatarUrlData(theAvatarValue)) {
                                            ((Activity) c).runOnUiThread(new Runnable() {
                                                @Override
                                                public void run() {
                                                    new generatePictureStyleNotification_big_text_with_large_icon(c, Integer.parseInt(uniqueID), Notif_title_queries[finalI], bigText_queries[finalI], theAvatarValue, bigText_queries[finalI], View_NDS_posts_comments.class, "the_post_id", sumMary_queries[finalI] + "-" + "comments").execute();
                                            ...
                                            ((Activity) c).runOnUiThread(new Runnable() {
                                                @Override
                                                public void run() {
                                                    new generatePictureStyleNotification_big_text_with_large_icon(c, Integer.parseInt(uniqueID), Notif_title_queries[finalI], bigText_queries[finalI], Config.LOAD_MY_AVATAR.toString() + "/" + theAvatarValue, bigText_queries[finalI], View_NDS_posts_comments.class, "the_post_id", sumMary_queries[finalI] + "-" + "comments").execute();

                                                }
                                            });

                                        }
                                    }catch (RuntimeException hsbsf){
    ...
    private Boolean parseData() {
        try {
            ...
            for (int counter = 0; counter < posts_Comments_Array.length(); counter++) {
                ...
                Notif_title_queries[counter] = "Post Comment";
                //----
                //--
