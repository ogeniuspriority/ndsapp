 ...
 */
public class My_Community_Creds_changeWorkInfo extends AsyncTask<Void, Void, String> {
    Context c;
    String urlAddress;
    String MyRemoteId;
    String WorkInfo;
    ...
    public My_Community_Creds_changeWorkInfo(Context c, String urlAddress, String MyRemoteId, String WorkInfo, AlertDialog popupWindow_000, Activity act) {
        this.c = c;
        ...
        if (jsonData == null) {
           ...
           String myTr = jsonData.replaceAll("\\s", "");
            if(myTr.equalsIgnoreCase("1000")){
                if (db.Update_NDS_USER_CRED(MyRemoteId, "work_address", WorkInfo)) {
                    Toast.makeText(c,"Work Info edited Suggessfully!",Toast.LENGTH_SHORT).show();
    ...
    private String sendMYComment() throws UnsupportedEncodingException {
        ...
        HttpURLConnection con = My_Community_Creds_workInfo_Connector.connect(urlAddress, MyRemoteId,WorkInfo);
        if (con == null) {
            return null;
        }
        try {
