...
import com.ogeniuspriority.nds.nds.NDS_messages_respond;
 ...
 */
public class My_Q_S_Comments_data_parser_B extends AsyncTask<Void, Void, Boolean> {
    Context c;
    String jsonData;
    ...
    public My_Q_S_Comments_data_parser_B(Context c, String jsonData) {
        this.c = c;
                ...
                for (int i = 0; i < Notif_number_queries.length; i++) {
                    try {
                        ...
                        if (db.save_A_NOTIFICATION(uniqueID, Notif_title_queries[i],
                                ...
                                origin_remote_id_queries[i],"messages_from_institutions",Orient_queries[i]
                               )) {
                            final int finalI = i;
                            ...
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    Looper.prepare();
                                    try {
                                        final String theAvatarValue = new NDS_main().FindThisTrickyAvatar(origin_remote_id_queries[finalI], "notif_using_suggestion");
                                        if (new NDS_main().renderThisAvatarUrlData(theAvatarValue)) {
                                            ((Activity) c).runOnUiThread(new Runnable() {
                                                @Override
                                                public void run() {
                                                    new generatePictureStyleNotification_big_text_with_large_icon(c, Integer.parseInt(uniqueID), Notif_title_queries[finalI], bigText_queries[finalI], theAvatarValue, bigText_queries[finalI], NDS_messages_respond.class, "queryOrSuggId", sumMary_queries[finalI] + "-" + Orient_queries[finalI]).execute();
    ...
    private Boolean parseData() {
        try {
            ...
            for (int counter = 0; counter < posts_Comments_Array.length(); counter++) {
                ...
                Notif_title_queries[counter] = posts_Comments_Array
                        .getJSONObject(
                                counter)
                        .getString(
